...
def render(db):
    ...
    if getattr(db, 'role', '') == 'admin':
        ...
        # --- TOMBOL AJAIB UNTUK MENGELUARKAN MATA PELAJARAN OTOMATIS ---
        if st.button("🔄 Keluarkan Mata Pelajaran Otomatis"):
            # Mengosongkan data lama agar terganti dengan template baru
            pengaturan_lama["kelas_mapel"] = {}
            db.simpan_pengaturan(pengaturan_lama)
            st.rerun()

        # Logika pemuatan template
        if not teks_awal:
        ...
        with st.expander("⚙️ Edit Daftar Kelas & Mata Pelajaran", expanded=True if not kelas_mapel else False):
            with st.form("form_mapel"):
                st.info("💡 **Petunjuk:** Jika sudah sesuai, langsung klik 'Simpan Mata Pelajaran' di bawah.")
    ...
    if getattr(db, 'role', '') == 'admin':
        ...
        with t_daftar:
            if daftar_guru:
                ...
                df_display.rename(columns={'nama_guru': 'Nama Lengkap', 'username': 'Username/NIP', 'role': 'Akses', 'kelas_binaan': 'Wali Kelas', 'jenis_kelamin': 'L/P', 'no_hp': 'No. WhatsApp'}, inplace=True)
